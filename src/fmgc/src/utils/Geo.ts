...
export class Geo {
    ...
    static distanceToLeg(from: Coordinates, leg: Leg): NauticalMiles {
        return Geo.getDistance(from, Geo.doublePlaceBearingIntercept(from, leg.terminator, leg.bearing - 90, leg.bearing - 180));
    }

    static legIntercept(from: Coordinates, bearing: DegreesTrue, leg: Leg): Coordinates {
        return Geo.doublePlaceBearingIntercept(from, leg.terminator, bearing, leg.bearing - 180);
    }
