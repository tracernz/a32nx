...
void AutopilotLawsModelClass::step()
{
  ...
  switch (static_cast<int32_T>(rtb_ManualSwitch)) {
   ...
   case 3:
    rtb_Y_fh = 180.0 / (39.478417604357432 * AutopilotLaws_P.zeta_Value * t) * (AutopilotLaws_P.Gain_Gain_c *
    ...
    rtb_Saturation1 = AutopilotLaws_U.in.data.flight_guidance_phi_deg - AutopilotLaws_P.zeta_Value / (215666.565757755 *
      t) * (AutopilotLaws_P.Gain2_Gain * AutopilotLaws_U.in.data.flight_guidance_tae_deg + rtb_Y_fh) *
  ...
  if (AutopilotLaws_U.in.data.H_radio_ft <= AutopilotLaws_P.CompareToConstant_const_d) {
    rtb_Y_fh = AutopilotLaws_P.Gain_Gain_a * rtb_out_h;
  } else {
  ...
  switch (static_cast<int32_T>(rtb_ManualSwitch)) {
   ...
   case 3:
    rtb_Y_p = AutopilotLaws_P.beta_Value_i;
    break;
  ...
  t = AutopilotLaws_P.Gain_Gain_o * rtb_Mod2_k + rtb_Saturation1;
  ...
  rtb_Saturation1 = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_e1 * AutopilotLaws_P.Gain_Gain_eq +
    rtb_Y_fh;
  AutopilotLaws_SpeedProtectionMode(&AutopilotLaws_Y.out, rtb_Divide_h, rtb_Y_p, t, AutopilotLaws_P.Gain_Gain_of * t,
    rtb_Saturation1, AutopilotLaws_P.Gain_Gain_mw * rtb_Saturation1, &rtb_Mod2_k, &rtb_out_h);
  ...
  AutopilotLaws_VSLimiter(AutopilotLaws_P.Gain_Gain_c3 * rtb_Divide_h, AutopilotLaws_U.in.data.V_tas_kn, &rtb_Y_p);
  ...
  rtb_Y_ic = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_p2 * AutopilotLaws_P.Gain_Gain_n1 + rtb_Y_fh;
  ...
  AutopilotLaws_SpeedProtectionMode(&AutopilotLaws_Y.out, rtb_Divide_h, rtb_Y_p, rtb_Y_ic, AutopilotLaws_P.Gain_Gain_i *
    rtb_Y_ic, rtb_Saturation1, AutopilotLaws_P.Gain_Gain_e5 * rtb_Saturation1, &rtb_Mod2_f, &t);
  ...
  if ((!AutopilotLaws_DWork.wasActive_g) && rtb_OR) {
    ...
    if (rtb_Saturation1 < 0.0) {
      ...
      rtb_Y_fh = rtb_Saturation1;
    }

    rtb_Saturation1 += rtb_Y_fh * AutopilotLaws_DWork.dH_offset;
    AutopilotLaws_DWork.k = AutopilotLaws_U.in.data.H_dot_ft_min / rtb_Saturation1;
  }
  ...
  rtb_Y_fh = (AutopilotLaws_DWork.k * rtb_Saturation1 - AutopilotLaws_U.in.data.H_dot_ft_min) *
    AutopilotLaws_P.ftmintoms_Gain_g / rtb_Y_fh;
  if (rtb_Y_fh > 1.0) {
    rtb_Y_fh = 1.0;
  } else {
    if (rtb_Y_fh < -1.0) {
  ...
  rtb_Sum2_bm = ((1.0 - (u1 - 1.0) * 0.1111111111111111) * AutopilotLaws_P.GammaTCorrection_gain * (1.0 /
  ...
  rtb_Sum2_i = (rtb_Gain_ny + rtb_Saturation1) * AutopilotLaws_P.Gain_Gain_nub * AutopilotLaws_P.Gain_Gain_ao + rtb_Y_fh;
  ...
  AutopilotLaws_Voter1(rtb_Sum_j, AutopilotLaws_P.Gain_Gain_b2 * rtb_Sum2_i, AutopilotLaws_P.VS_Gain_c * rtb_Gain_a3,
                       &rtb_Saturation1);
  switch (static_cast<int32_T>(rtb_GainTheta1)) {
   ...
   case 3:
    if (dH_tmp > AutopilotLaws_P.Switch_Threshold_n) {
      rtb_Y_fh = AutopilotLaws_P.Gain_Gain_kg * rtb_Sum2_bm;
  ...
  AutopilotLaws_Voter1(rtb_Sum_j, rtb_Sum2_i, rtb_Gain_a3, &rtb_Gain_ny);
  switch (static_cast<int32_T>(rtb_GainTheta1)) {
   case 0:
